...
正誤表
https://lowlayergirls.github.io/wasabi-help/os1/errata
...
p.110
一回メモリマップを取得してからUEFIを抜ける時までの間にメモリマップが変わる可能性があるってことかな？だから再度取得してるのかな。

p.146
アロケータを作る。記述量がめっちゃあるけど落ち着いて読めばやってる事自体はシンプル。

p.164
`SerialPort::init`はどこからも呼ばれてないように見えるけど、この初期化は不要なのか…？ `panic!()`入れてみたけどやっぱり呼ばれてなさそう。

`write_str`はなんで`self`を使わずに新しく生成してるんだろう

p.168
アロケータのテスト。これも記述量が多くてhaten面倒だけど、アロケータがバグってると後々もっと面倒なのでちゃんと書いておく。